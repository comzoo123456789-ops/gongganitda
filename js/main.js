...
$("#catGrid").innerHTML = CATEGORIES.map((c) => `
  <button class="cat" data-cat="${c.id}">
    <span class="cat__ic" style="background:${c.tint};color:${c.ink}">${iconSVG(c.icon, 26)}</span>
  ...
  return `<article class="sp-card" data-id="${s.id}">
    ...
    <div class="sp-card__body">
      ...
      <div class="sp-card__meta">
        <span>${iconSVG("pin", 15)}${s.region.replace("서울 ", "")}</span>
        <span>${iconSVG("users", 15)}~${s.capacity}인</span>
      </div>
...
(function flagship() {
  ...
  $("#flag").innerHTML = `<div class="flag__inner">
    <div class="flag__media" style="background:linear-gradient(135deg,${f.g[0]},${f.g[1]})">
      <span class="flag__tag">OFFICIAL · 직영</span>
      <span class="flag__mediatitle">${f.name}</span>
    </div>
    <div class="flag__body">
      <span class="flag__eyebrow">공간잇다 직영 공간</span>
      <h2 class="flag__title">모임·행사, 여기서 시작하세요</h2>
      ...
      <a href="#" class="btn btn--accent btn--lg" data-id="${f.id}" style="align-self:flex-start">예약 가능 시간 보기 →</a>
    </div>
  </div>`;
})();
...
document.addEventListener("click", (e) => {
  ...
  // 카드/예약 버튼 (상세 페이지는 다음 단계)
  const card = e.target.closest(".sp-card, [data-id]");
  if (card && card.dataset.id) { toast("공간 상세·예약 페이지는 다음 단계에서 열려요 🙌"); }
});
...
// 햄버거
const hm = $("#hmenu"), nav = $(".header__nav");
